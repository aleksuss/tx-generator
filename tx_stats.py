...
def parse_datetime(d_time):
    return datetime.strptime(d_time, '%Y-%m-%dT%H:%M:%S.%fZ')
...
def update_stats(stats, data):
    ...
    for i, block in enumerate(data['blocks']):
        ...
        if tx_count == 0:  # skip blocks with no transactions
            continue
        if i == len(data['blocks']) - 1:  # skip last block info, we won't be able to calculate time delta
            continue
        block_time = (parse_datetime(times[i]) - parse_datetime(times[i + 1])).total_seconds()
...
def main():
    ...
    blocks_url = '{}/api/explorer/v1/blocks?count={}&add_blocks_time=true'.format(hostname, count_blocks)
    stats = dict()
    ...
    while True:
        try:
            ...
            if response.status_code == 200:
                ...
                print("min: {}, max: {}, avrg: {}, current: {}, last height: {}"
                      .format(min_tps, max_tps, avrg_tps, current_tps, last_height), end="\r")
            else:
                print('Bad request')
